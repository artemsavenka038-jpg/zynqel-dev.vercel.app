...
// =====================================================
// ZYNQEL ACTIVE MODE MY ACTION PRECISE FIX
// Safe patch: does not touch DOM. Only changes returned trade decision
// when Active Mode has a strong early setup.
// =====================================================
(function(){
  if(window.__ZYNQEL_ACTIVE_MY_ACTION_PRECISE_FIX__) return;
  window.__ZYNQEL_ACTIVE_MY_ACTION_PRECISE_FIX__ = true;
  ...
  function candles(asset){
    var f = frames(asset);
    if(f['15m'] && f['15m'].length >= 25) return f['15m'];
    if(f['1h'] && f['1h'].length >= 25) return f['1h'];
    return [];
  }
  ...
  function activeSetup(asset, forecast){
    ...
    var price = num(live(asset).price, null);
    ...
    var earlyScore = early(asset);

    var bullish = price && e9 && e21 && price > e9 && e9 >= e21;
    var bearish = price && e9 && e21 && price < e9 && e9 <= e21;

    // Active accepts neutral probability if the live structure is strong.
    var buy = prob >= 48 && conf >= 56 && bullish && earlyScore >= 70 && vol >= 0.75;
    var sell = prob <= 52 && conf >= 56 && bearish && earlyScore >= 70 && vol >= 0.75;

    if(buy){
      return {side:'buy', price:price, prob:prob, conf:conf, early:earlyScore, vol:vol};
    }
    if(sell){
      return {side:'sell', price:price, prob:prob, conf:conf, early:earlyScore, vol:vol};
    }
    return null;
  }

  function makeActiveTrade(asset, setup, oldTrade){
    var ru = window.appLang === 'ru';
    var p = setup.price;
    var low = setup.side === 'buy' ? p*0.998 : p*1.002;
    var high = setup.side === 'buy' ? p*1.006 : p*0.994;
    var invalid = setup.side === 'buy' ? p*0.986 : p*1.014;

    var entryZone = fmtPrice(Math.min(low, high)) + ' — ' + fmtPrice(Math.max(low, high));
    var title = setup.side === 'buy'
    ...
    var entry = setup.side === 'buy'
      ? (ru ? 'Активная зона: ' : 'Active zone: ') + entryZone + (ru ? ' · вход только после удержания EMA и объёма' : ' · enter only after EMA hold and volume confirmation')
      : (ru ? 'Активная зона шорта: ' : 'Active short zone: ') + entryZone + (ru ? ' · вход только после пробоя и объёма' : ' · enter only after breakdown and volume confirmation');

    var risk = (ru ? 'Отмена идеи: ' : 'Invalidation: ') + fmtPrice(invalid) +
      ' · Active Mode: Early ' + Math.round(setup.early) + '/100, Vol x' + setup.vol.toFixed(2) +
      ', Confidence ' + Math.round(setup.conf) + '%';
    ...
    return {
      ...
      entryText: entry,
      riskText: risk,
      note: oldTrade && oldTrade.note ? oldTrade.note : (ru ? 'Это не финансовая рекомендация, а сигнал приложения по текущим данным.' : 'This is not financial advice; it is an app signal based on current data.'),
      source: ((oldTrade && oldTrade.source) || 'LOCAL ENGINE') + ' + ACTIVE MODE'
    };
  }

  function wrapBuildTradeDecision(){
    if(typeof window.buildTradeDecision !== 'function') return false;
    if(window.buildTradeDecision.__activeMyActionPreciseFix) return true;

    var oldBuild = window.buildTradeDecision;
    window.buildTradeDecision = function(assetId, forecast){
      var oldTrade = oldBuild.apply(this, arguments);
      try{
        var setup = activeSetup(assetId || window.currentAnalysisAsset || window.currentAsset || 'BTC', forecast || window.lastAnalysisForecast || {});
        if(setup) return makeActiveTrade(assetId, setup, oldTrade);
      }catch(e){
        console.warn('Active My Action fix failed:', e.message);
      }
      return oldTrade;
    };
    window.buildTradeDecision.__activeMyActionPreciseFix = true;
    return true;
  }

  wrapBuildTradeDecision();
  setTimeout(wrapBuildTradeDecision, 500);
  setTimeout(wrapBuildTradeDecision, 1500);
  setInterval(wrapBuildTradeDecision, 2500);

  console.log('✅ ZYNQEL Active My Action Precise Fix enabled');
})();
