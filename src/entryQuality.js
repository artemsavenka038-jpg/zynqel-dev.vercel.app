...
(function(){
  ...
  function buildEntryQuality(id){
    ...
    // Safe / Active thresholds.
    // SAFE stays strict. ACTIVE allows early entries when probability is neutral
    // but candles/EMA/volume/early impulse are strong.
    ...
    var buyActive =
      ...
      volumeBoost >= 0.75 &&
      earlyScore >= 70 &&
      !(orderBook !== null && orderBook < -12) &&
      !(taker !== null && taker < 0.86);
    ...
    if(buyValid){
      ...
      if(extended){
        ...
        state = (activeMode && !buyStrict) ? 'EARLY ENTRY' : 'STRONG ENTRY';
        color = (activeMode && !buyStrict) ? 'var(--gold)' : 'var(--green)';
        emoji = (activeMode && !buyStrict) ? '🟡' : '🟢';
        reasons.push((activeMode && !buyStrict)
          ? (ru() ? 'ACTIVE: ранний BUY, импульс сильный, но вход только после подтверждения' : 'ACTIVE: early BUY setup, momentum is strong, but enter only after confirmation')
      ...
      if(extended){
        ...
        state = (activeMode && !sellStrict) ? 'EARLY SHORT' : 'STRONG ENTRY';
        color = (activeMode && !sellStrict) ? 'var(--gold)' : 'var(--red)';
        emoji = (activeMode && !sellStrict) ? '🟡' : '🔴';
        reasons.push((activeMode && !sellStrict)
          ? (ru() ? 'ACTIVE: ранний SELL, давление сильное, но вход только после подтверждения' : 'ACTIVE: early SELL setup, pressure is strong, but enter only after confirmation')
