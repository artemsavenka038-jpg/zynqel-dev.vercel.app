...
(function(){
  ...
  function getRenderedForecast(){
    ...
    var modeNow = (typeof window.zynqelGetTradeMode === 'function')
      ? window.zynqelGetTradeMode()
      : (localStorage.getItem('zynqel_trade_mode') || 'safe');
    if(modeNow !== 'active' && (f.isActiveModeSignal || String(f.source || '').indexOf('ACTIVE') >= 0)){
      action = 'wait';
    }
    ...
    if(!action){
      if(modeNow === 'active'){
        if(/i would buy|i would prepare for early buy|я бы купил|готовился к раннему buy/i.test(txt)) action = 'buy';
        else if(/i would sell|i would prepare for early sell|я бы продал|готовился к раннему sell/i.test(txt)) action = 'sell';
        else action = 'wait';
      }else{
        if(/i would buy|я бы купил/i.test(txt) && !/early buy|раннему buy/i.test(txt)) action = 'buy';
        else if(/i would sell|я бы продал/i.test(txt) && !/early sell|раннему sell/i.test(txt)) action = 'sell';
        else action = 'wait';
      }
    }
  ...
  function buildEntryQuality(id){
    ...
    if(buyValid){
      ...
      if(activeMode && !buyStrict){
        ...
        state = (activeMode && !buyStrict) ? 'WATCH ENTRY' : 'STRONG ENTRY';
        color = (activeMode && !buyStrict) ? 'var(--gold)' : 'var(--green)';
        emoji = (activeMode && !buyStrict) ? '🟡' : '🟢';
        reasons.push((activeMode && !buyStrict)
          ? (ru() ? 'ACTIVE: ранний BUY, вход только после свечи/объёма' : 'ACTIVE: early BUY setup exists, but enter only after candle/volume confirmation')
          : (ru() ? 'EMA структура бычья, объём поддерживает вход' : 'EMA structure is bullish and volume supports entry'));
      }
    }else if(sellValid){
      direction = 'short';
      if(activeMode && !sellStrict){
        ...
        state = (activeMode && !sellStrict) ? 'WATCH SHORT' : 'STRONG ENTRY';
        color = (activeMode && !sellStrict) ? 'var(--gold)' : 'var(--red)';
        emoji = (activeMode && !sellStrict) ? '🟡' : '🔴';
        reasons.push((activeMode && !sellStrict)
          ? (ru() ? 'ACTIVE: ранний SELL, вход только после свечи/объёма' : 'ACTIVE: early SELL setup exists, but enter only after candle/volume confirmation')
          : (ru() ? 'EMA структура медвежья, давление продавцов подтверждается' : 'EMA structure is bearish and seller pressure is confirmed'));
      }
    }else{
      state = 'NO ENTRY';
      color = 'var(--muted)';
  ...
  function renderEntryQuality(){
    try{
      ...
      var card = document.getElementById('zynqel-entry-quality-card');
      if(card){
        // Update the existing card only. Do not remove/reinsert it.
        // This prevents flicker, duplicated answers, and jumping blocks.
        if(card.innerHTML !== inner) card.innerHTML = inner;
        return;
      }

      card = document.createElement('div');
      card.id = 'zynqel-entry-quality-card';
...
// =====================================================
// ZYNQEL DATA QUALITY GUARD
// Prevents fake P 50% / C 50% from looking like a real signal when data is incomplete.
// =====================================================
(function(){
  if(window.__ZYNQEL_DATA_QUALITY_GUARD__) return;
  window.__ZYNQEL_DATA_QUALITY_GUARD__ = true;
  window.zynqelForecastDataQuality = function(fc){
    fc = fc || window.lastAnalysisForecast || {};
    var hasProb = Number.isFinite(Number(fc.probability ?? fc.upwardProbability ?? fc.prob));
    var hasConf = Number.isFinite(Number(fc.confidence));
    return {hasProb:hasProb,hasConf:hasConf,ok:hasProb&&hasConf,prob:hasProb?Number(fc.probability ?? fc.upwardProbability ?? fc.prob):null,conf:hasConf?Number(fc.confidence):null};
  };
  var oldRender = window.zynqelRenderEntryQuality;
  if(typeof oldRender === 'function' && !oldRender.__dataQualityGuard){
    window.zynqelRenderEntryQuality = function(){
      var out = oldRender.apply(this, arguments);
      try{
        var q=window.zynqelForecastDataQuality(window.lastAnalysisForecast);
        var card=document.getElementById('zynqel-entry-quality-card')||document.getElementById('entry-quality-card')||document.getElementById('entryQualityCard');
        if(card && !q.ok){
          card.setAttribute('data-zynqel-data-quality','incomplete');
          if((card.textContent||'').indexOf('DATA CHECK')<0){
            card.innerHTML='<div class="zq-entry-title">ENTRY QUALITY</div><div class="zq-entry-main" style="color:var(--gold);">🟡 DATA CHECK</div><div class="zq-entry-reason">Forecast probability/confidence is incomplete. Waiting for full local/Groq calculation.</div><div class="zq-entry-small">This prevents fake P 50% / C 50% from looking like a real signal.</div>';
          }
        }
      }catch(e){}
      return out;
    };
    window.zynqelRenderEntryQuality.__dataQualityGuard = true;
  }
})();
