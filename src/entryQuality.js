...
// ZYNQEL hard entry quality slot hook
(function(){
  if(window.__ZYNQEL_HARD_ENTRY_QUALITY_HOOK__) return;
  window.__ZYNQEL_HARD_ENTRY_QUALITY_HOOK__ = true;
  function run(){
    try{
      if(typeof window.zynqelFixEntryQualitySlot === 'function'){
        window.zynqelFixEntryQualitySlot();
      }
    }catch(e){}
  }
  setTimeout(run, 50);
