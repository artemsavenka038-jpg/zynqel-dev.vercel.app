...
function buildTradeDecision(assetId, forecast){
  ...
  try{
    var activeMode = (typeof window.zynqelGetTradeMode === 'function')
    ...
    if(activeMode && action === 'wait'){
      ...
      if(conf >= 56 && earlyScore >= 70 && bullish){
        ...
        forecast.source = (source || 'LOCAL ENGINE') + ' + ACTIVE MODE';
        forecast.entryZone = (isRu ? 'Active BUY watch zone: ' : 'Active BUY watch zone: ') + fmtPrice(price*0.994) + ' — ' + fmtPrice(price*1.006);
        forecast.invalidation = fmtPrice(price*0.986);
        forecast.target1 = fmtPrice(price*1.014);
        forecast.waitFor = isRu ? 'ACTIVE: жду подтверждение EMA/свечи и объёма' : 'ACTIVE: waiting for EMA/candle and volume confirmation';
      }else if(conf >= 56 && earlyScore >= 70 && bearish){
        action = 'sell';
        activePrepareSide = 'sell';
        forecast.source = (source || 'LOCAL ENGINE') + ' + ACTIVE MODE';
        forecast.entryZone = (isRu ? 'Active SELL watch zone: ' : 'Active SELL watch zone: ') + fmtPrice(price*0.994) + ' — ' + fmtPrice(price*1.006);
        forecast.invalidation = fmtPrice(price*1.014);
        forecast.target1 = fmtPrice(price*0.986);
        forecast.waitFor = isRu ? 'ACTIVE: жду пробой/удержание ниже EMA и объём' : 'ACTIVE: waiting for breakdown/hold below EMA and volume';
      }
    }
  ...
  var entryZone=cleanDecisionField(forecast && forecast.entryZone);
  var inval=cleanDecisionField(forecast && forecast.invalidation);
  var target=cleanDecisionField(forecast && forecast.target1);
  var entryText;
  ...
  if(action!=='wait' && (inval || target)){
    ...
    var waitFor=cleanDecisionField(forecast && (forecast.waitFor || forecast.confirmation || forecast.trigger));
...
function updateActiveAnalysisLive(){
  try{
    ...
    if(lastAnalysisForecast && lastAnalysisAsset===currentAnalysisAsset){
      var trade=buildTradeDecision(currentAnalysisAsset,lastAnalysisForecast);
