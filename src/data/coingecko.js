...
function buildTradeDecision(assetId, forecast){
  ...
  // SAFE guard: never display stale ACTIVE result after switching mode.
  var __safeModeNow = true;
  try{
    __safeModeNow = typeof window.zynqelGetTradeMode === 'function'
      ? window.zynqelGetTradeMode() !== 'active'
      : localStorage.getItem('zynqel_trade_mode') !== 'active';
  }catch(e){ __safeModeNow = true; }

  if(__safeModeNow && forecast && (forecast.isActiveModeSignal || String(source).indexOf('ACTIVE') >= 0)){
    action = 'wait';
    source = String(source || 'LOCAL ENGINE').replace(/\s*\+\s*ACTIVE MODE/g,'').replace(/\s*\+\s*ACTIVE/g,'');
  }
